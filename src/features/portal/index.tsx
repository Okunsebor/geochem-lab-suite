...
export function PortalDashboardFeature() {
  ...
  const activeCount = samples.filter(
    (s) => s.client === tenantClient && s.status !== "Completed" && s.status !== "Report Ready",
  ).length;
  const readyReports = reports.filter(
    (r) => r.client === tenantClient && r.status === "Approved",
  ).length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Portal3DCard
          className="lg:col-span-2 rounded-xl border border-border bg-card/90 backdrop-blur-md"
          delay={0.15}
        >
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-mono text-xs text-primary font-medium hover:underline">
                          {s.id}
                        </p>
        ...
        <Portal3DCard
          className="rounded-xl border border-border bg-card/90 backdrop-blur-md p-5 h-fit"
          delay={0.2}
        >
            ...
            <ul className="mt-3 space-y-2 max-h-[350px] overflow-y-auto">
              {reports
                ...
                .map((r) => (
                  <li
                    key={r.id}
                    className="flex items-center justify-between rounded border border-border p-2.5 text-xs bg-card"
                  >
                    <div>
                      <p className="font-mono text-primary font-semibold">{r.id}</p>
                      <p className="text-muted-foreground mt-0.5 font-medium">
                        {r.pages} pages · Certified
                      </p>
                    </div>
                    <button
                      onClick={() => {
              ...
              {readyReports === 0 && (
                <p className="text-xs text-muted-foreground text-center py-4 font-medium">
                  No certificates pending download
                </p>
              )}
            </ul>
          </div>
          ...
          <div className="w-full max-w-md bg-card border-l border-border h-full flex flex-col shadow-2xl animate-in slide-in-from-right duration-300">
            <div className="flex items-center justify-between px-6 py-4 border-b border-border bg-muted/30">
              ...
              <button
                ...
                className="p-2 rounded-md hover:bg-muted text-muted-foreground transition cursor-pointer"
              >
                <X className="size-5" />
              </button>
            </div>
                ...
                <div className="space-y-6 relative before:absolute before:inset-0 before:ml-[11px] before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-border before:to-transparent">
                  {SAMPLE_STATUSES.map((status, index) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`flex items-center justify-center w-6 h-6 rounded-full border-2 bg-card shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 shadow z-10 ${
                            isCompleted
                        ...
                        >
                          {isCompleted ? (
                            ...
                            <div
                              className={`size-1.5 rounded-full ${isCurrent ? "bg-primary" : "bg-transparent"}`}
                            />
                          )}
                        </div>
                        <div className="w-[calc(100%-3rem)] md:w-[calc(50%-2rem)]">
                          <div
                            className={`p-3 rounded-lg border ${isCurrent ? "border-primary bg-primary/5" : "border-border bg-card"} shadow-sm`}
                          >
                            <p
                              className={`text-sm font-semibold ${isCurrent ? "text-primary" : "text-foreground"}`}
                            >
                              {status}
                            </p>
                    ...
                    {selectedSample.custody.map((log, i) => (
                      <div key={i} className="text-xs border-l-2 border-primary/30 pl-3 py-1">
                        ...
                        <p className="text-muted-foreground mt-0.5">
                          {new Date(log.time).toLocaleString()} · {log.technician}
                        </p>
                      </div>
                    ))}
                  </div>
    ...
    return (
      <div className="space-y-6 max-w-2xl mx-auto mt-8 text-center animate-in fade-in zoom-in duration-500">
        ...
        <p className="text-muted-foreground mt-2">
          Your sample has been pre-registered. Please print the barcode below and attach it to your
          physical shipment.
        </p>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Order test requests and pre-register barcodes.
        </p>
      </div>

      <form
        onSubmit={handleSubmit}
        className="rounded-xl border border-border bg-card p-6 space-y-5 max-w-2xl"
      >
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {mine.map((r) => (
                <tr
                  key={r.id}
                  className="border-t border-border [&>td]:px-4 [&>td]:py-2.5 font-medium transition-colors hover:bg-muted/10"
                >
...
export function PortalNotificationsFeature() {
  const { notifications, unreadNotificationCount, markAllNotificationsRead, markNotificationRead } =
    useLimsState();
        ...
        {notifications.map((n) => (
          <li key={n.id} className="p-4 text-sm hover:bg-muted/10 transition-colors">
            <div className="flex items-center justify-between gap-3">
              <p
                className={`font-medium ${n.isRead ? "text-muted-foreground" : "text-foreground"}`}
              >
                {n.title}
              </p>
              {!n.isRead && (
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Submit technical questions or request analytical reruns.
        </p>
      </div>
        ...
        <div className="rounded-xl border border-border bg-card p-5 h-fit space-y-4">
          ...
          <ul className="space-y-2 text-xs">
            {allTickets.map((t) => (
              <li
                key={t.id}
                className="rounded border border-border p-2.5 bg-card flex justify-between items-center font-medium"
              >
                <div>
