...
export function QaqcStatsBar({ passRate, openFlagCount, crmOutOfSpec, avgDuplicateSpread }: QaqcStatsBarProps) {
  const stats = [
    {
      label: "Pass Rate",
      value: `${passRate}%`,
      icon: ShieldCheck,
      color: passRate >= 95 ? "hsl(158 64% 48%)" : passRate >= 85 ? "hsl(38 95% 55%)" : "hsl(0 84% 55%)",
      bg:    passRate >= 95 ? "bg-emerald-500/10"  : passRate >= 85 ? "bg-amber-500/10"  : "bg-rose-500/10",
    },
    {
      label: "Open Flags",
      value: String(openFlagCount),
      icon: AlertTriangle,
      color: openFlagCount === 0 ? "hsl(158 64% 48%)" : openFlagCount <= 3 ? "hsl(38 95% 55%)" : "hsl(0 84% 55%)",
      bg:    openFlagCount === 0 ? "bg-emerald-500/10" : openFlagCount <= 3 ? "bg-amber-500/10"  : "bg-rose-500/10",
    },
    {
    ...
    },
    {
      ...
      icon: Activity,
      color: avgDuplicateSpread == null || avgDuplicateSpread <= 10 ? "hsl(210 90% 55%)" : "hsl(38 95% 55%)",
      bg:    "bg-primary/10",
    },
  ];
